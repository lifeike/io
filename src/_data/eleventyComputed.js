  ...
  shopProducts: (data) => {
    ...
    function getSimplePrice(priceRange) {
      ...
      if (!formattedMinPrice) {
        return null
      } else if (priceRange.minVariantPrice.amount === priceRange.maxVariantPrice.amount) {
    ...
    return data.shopify?.products.map((product) => {
      ...
      const wasPrice = Number(product.compareAtPriceRange.minVariantPrice.amount)
      const discountedPrice = Number(product.priceRange.minVariantPrice.amount)
      let saleAmount = null
      if (wasPrice !== discountedPrice && wasPrice !== 0) {
        saleAmount = (((wasPrice - discountedPrice) / wasPrice) * 100).toFixed(0)
      }
